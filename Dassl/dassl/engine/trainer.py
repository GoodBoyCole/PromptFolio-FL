...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        # info list for global and local
        self.local_info = [{} for i in range(self.cfg.DATASET.USERS)]
    ...
    def build_data_loader(self):
        ...
        self.ori_train_loader_x_dict = dm.ori_train_loader_x_dict
        self.datum_store = []
    ...
    def fed_init_model(self, init_weight):
        '''
        This method is implemented to initialize the local info list.
        '''
        for i in range(self.cfg.DATASET.USERS):
            self.local_info[i] = copy.deepcopy(init_weight)

    def fed_upload_model(self, idx):
        '''
        This method is implemented to obtain the local information
        '''
        self.local_info[idx] = copy.deepcopy(self.model.state_dict())

    def fed_aggregate_model(self, idx_users):
        '''
        This method is implemented to aggregate the local information
        '''
        self.global_info = copy.deepcopy(self.average_weights(self.local_info, idx_users))

    def fed_download_model(self, idx):
        '''
        This method is implemented to retrieve the local information and obtain the global information
        '''
        self.model.load_state_dict(self.local_info[idx])
        if self.global_info is not None:
            self.model.load_state_dict(self.global_info, strict=False)



    def average_weights(self, w, idxs_users, islist=False):
        """
        ...
        datanumber_client = []
        for i in range(self.cfg.DATASET.USERS):
            datanumber_client.append(len(self.fed_train_loader_x_dict[i].dataset))
...
class TrainerX(SimpleTrainer):
    ...
    def zero_shot_inference(self, model, eval_loader, num_class, batch_size, feat_dim=768, budget=1., sup_label=None,
              reg_feat=0.5, reg_lab=0.5, version='fast', Pmode='out', reg_e=0.01, reg_sparsity=None):
        """
        This method use pretrained CLIP to conduct zero-shot inference on noisy label.
        @param model:
        @param eval_loader:
        @param num_class:
        @param batch_size:
        ...
        @param reg_sparsity:
        @return:
        all_pseudo_labels: The pseudo labels generated by zero shot inference
        all_noisy_labels: The original noisy label
        all_gt_labels: The original clean label
        all_selected_mask: The samples that have high confidence = (maximum of the logit value) will be chosed to train.
        all_argmax_plabels: The argmax of the pseudo labels
        """

...
class OfficeDataset(Dataset):
    ...
    def _convert(self):
        data_with_label = []
        for i in range(len(self.target)):
            # base_path + office_caltech_10/amazon/headphones/frame_0035.jpg
            img_path = os.path.join(self.base_path, "/".join(self.paths[i].split("/")[-4:]))
            data_idx = img_path
            target_idx = self.target[i]
            label_idx = self.label[i]
            item = Datum(impath=data_idx, label=int(target_idx), domain=int(self.domain), classname=label_idx)
...
class DomainNetDataset(Dataset):
    ...
    def _convert(self):
        data_with_label = []
        for i in range(len(self.target)):
            # base_path + DomainNet/painting/zebra/painting_344_000273.jpg
            img_path = os.path.join(self.base_path, "/".join(self.paths[i].split("/")[-4:]))
            data_idx = img_path
            target_idx = self.target[i]
            label_idx = self.label[i]
